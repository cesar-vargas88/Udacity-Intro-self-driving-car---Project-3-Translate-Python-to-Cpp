...
// your code here
	...
	if (grid.size() > 0)
	{
		vector < vector <float> > newGrid(grid.size(), vector<float>(grid[0].size(), (float) (1.0 / (grid.size() * grid[0].size()))));		
		return newGrid;
	}
	else
		return vector < vector <float> >(1, vector<float>(1, 0.0));
}
	...
	float p_miss) 
{
	...
	if (beliefs.size() > 0) {
		...
		for (int i = 0; i < height; i++) {
			for (int j = 0; j < width; j++)
				if (grid[i][j] == color)
					newGrid[i][j] = p_hit / sum;
				else
					newGrid[i][j] = p_miss / sum;
		}
